...
# Packages token is available) and lands at target/out/jvm/scala-<ver>/<project>/dicechess-bot-gcp.jar
# in sbt 2.x — see .dockerignore — so this Docker build needs no registry credentials.
# Base is a JRE >= 25 because dicechess-bot-runtime is compiled to Java 25 bytecode.
