# Dice Chess Bot — Google Cloud Run (Scala Monte-Carlo) — AI Agent Guidelines

## Architecture Overview
- **Domain**: Production Dice Chess webhook bot running the engine's Monte-Carlo rollout search on the JVM in Google Cloud Run.
- **Dependencies**: Links `@fortemate/dicechess-engine` (MonteCarloSearch, FenParser, TimeManager) and `dicechess-bot-runtime` (CustomHandlerServer, HMAC-SHA256 verification).
- **Time Management**: Automatically budgeted from remaining game clocks via `TimeManager`.

## Developer Workflows
- **Build**: `sbt compile` (Java 25+, Scala 3.8.4).
- **Tests**: `sbt "testOnly *"` (munit test suites).
- **Assembly**: `sbt assembly` (produces runnable fat jar `target/out/jvm/scala-3.8.4/dicechess-bot-gcp/dicechess-bot-gcp.jar`).
- **Container**: `docker build -t dicechess-bot-gcp .` (packages the assembled jar on `eclipse-temurin:25-jre`).
