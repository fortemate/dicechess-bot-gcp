...
[![CI](https://github.com/fortemate/dicechess-bot-gcp/actions/workflows/ci.yml/badge.svg)](https://github.com/fortemate/dicechess-bot-gcp/actions/workflows/ci.yml)
...
The live [`gcp/scala-monte-carlo`](https://fortemate.com/leaderboard) ladder bot: a Dice Chess
...
[`TimeManager`](https://github.com/fortemate/dicechess-engine): build a `ClockState` from the
...
so there is nothing to configure for it. Two tuning knobs remain, both with safe defaults:
...
| `src/main/scala/dicechess/bot/Main.scala` | Wires `Strategy` into [`dicechess-bot-runtime`](https://github.com/fortemate/dicechess-bot-runtime)'s `WebhookHandler`/`CustomHandlerServer`, binding Cloud Run's `$PORT`. |
